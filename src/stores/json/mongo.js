...
class MongoAdapter extends BaseAdapter {
  ...
  async update(collection, query, operations, opts = {}) {
    ...
    if (options.multi) {
      ...
      await store.updateMany(matchesIds, operations, { upsert: !!options.upsert });
      return store.find(matchesIds).toArray();
    }

    const result = await store.findOneAndUpdate(query, operations, {
  ...
  upsert(collection, query, operations, opts = {}) {
    return this.update(collection, query, operations, { ...opts, upsert: true });
  }
  ...
  async remove(collection, query, options = { multi: true }) {
    let store = this.db.collection(collection);
    let remove = promisify(
      options.multi ? store.deleteMany : store.deleteOne,
      store
    );
